...
fn first_star_inner(first_input: &str, second_input: &str) -> Result<i64, failure::Error> {
    // Get all coordinates.
    let first_coords = get_all_coordinates_from_input(first_input).unwrap();
    let second_coords = get_all_coordinates_from_input(second_input).unwrap();
...
fn get_all_coordinates_from_input(input: &str) -> Result<HashSet<(i64, i64)>, failure::Error> {
